    ...
    )
{
    ...
    if (!q_IsInitialized){
        Log(PDO_LOG_DEBUG, "InitializingQueue");
        initiliazeQueue();
    }
    ...
    try {
        ...
        if (method == "contract_verify_secrets"){
            ...
            sgx_enclave_id_t enclaveid = g_Enclave[enclaveIndex].GetEnclaveId();
            Log(PDO_LOG_DEBUG, "DoECall - VerifySecrets - [%u]Enclave_ID:  %ld ", enclaveIndex, (long)enclaveid);
            ...
            int enclaveIndex = queue.pop();
            sgx_enclave_id_t enclaveid = g_Enclave[enclaveIndex].GetEnclaveId();
            Log(PDO_LOG_DEBUG, "DoECall - HandleContractRequest/GetSerializedResponse - [%u]Enclave_ID:  %ld ", enclaveIndex, (long)enclaveid);
